...
const getHandler = async (
  ...
  try {
    ...
    if (partnerIds.length === 0 && directEntityIds.length === 0) {
      return json([]);
    }
    ...
    const allVisibleEntityIds = Array.from(
      new Set([...directEntityIds, ...entityIdsViaPartners])
    );

    if (allVisibleEntityIds.length === 0) {
      return json([]);
    }

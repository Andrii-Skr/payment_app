...
import { Combobox } from '@/components/shared/combobox';
 ...
 const formSchema = z.object({
  sample: z.string().min(2, {
    message: "Не придумал"
  }).optional(),
  accountNumber: z.string({message: "Пожалуйста, заполните это поле"}).min(2, {
    ...
    <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <Container className='justify-start gap-10'>
            ...
            <FormInput control={form.control} name="paySum" label="Сумма к оплате" description='Фактическая сумма оплаты, если 0, то сумма будет равна сумме счета'/>
          </Container>
          <Container className='justify-start gap-10' >
            <FormInput control={form.control} name="bankAccount" label="Номер счета" />
            <Combobox   />
          </Container>
          <FormInput control={form.control} name="partnerName" label="Имя контрагента" />
          <FormInput control={form.control} name="mfo" label="МФО" />
          <FormInput control={form.control} name="purposeOfPayment" label="Примечание" placeholder="Оплата по счету" description='Оплата по счету'/>
          <Button type="submit">Сохранить</Button>
          <Button type="submit" className="ml-8">Отправить на оплату</Button>
          </form>

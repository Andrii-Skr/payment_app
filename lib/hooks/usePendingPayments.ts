...
export const usePendingPayments = () => {
  ...
  const setPendingPayments = usePaymentStore((state) => state.setPendingPayments);
  const clearPendingPayments = usePaymentStore((state) => state.clearPendingPayments);

  const groupedPayments = useMemo(() => {
    return pendingPayments.reduce(
      (acc, p) => {
        if (!acc[p.partner_id]) acc[p.partner_id] = { name: p.partner_name, total: +p.pay_sum };
  ...
  const update = (newDetails: PaymentDetail[], existingIds: number[]) => {
    const filtered = pendingPayments.filter((p) => !existingIds.includes(p.spec_doc_id));
    ...
    if (hasPaid && hasUnpaid) {
      toast.error("Нельзя одновременно выбирать оплаченные и неоплаченные документы");
      return;
    }

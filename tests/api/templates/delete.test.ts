...
import * as handler from "@api/templates/delete/route";
...
describe("PATCH /templates/delete", () => {
  ...
  it("401 если нет сессии", async () => {
    ...
    await testApiHandler({
      appHandler: handler,
      test: async ({ fetch }) => {
        const res = await fetch({ method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ template_id: 1, is_deleted: true }) });
